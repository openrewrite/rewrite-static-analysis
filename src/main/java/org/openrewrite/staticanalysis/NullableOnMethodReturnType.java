...
public class NullableOnMethodReturnType extends Recipe {
        ...
        JavaIsoVisitor<ExecutionContext> visitor = new JavaIsoVisitor<ExecutionContext>() {
            ...
            public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
                ...
                return m.getLeadingAnnotations().stream()
                        .filter(NullableOnMethodReturnType::isNullabilityAnnotation)
                        .findFirst()
                        .map(nullabilityAnnotation -> {
                            J.MethodDeclaration m2 = m.withLeadingAnnotations(ListUtils.map(m.getLeadingAnnotations(),
                                    a -> a == nullabilityAnnotation ? null : a));
                            if (m2.getReturnTypeExpression() == null) {
                                return m2;
                            }
                            ...
                            if (m2.getReturnTypeExpression() instanceof J.ArrayType) {
                                ...
                                m2 = m2.withReturnTypeExpression(((J.ArrayType) m2.getReturnTypeExpression())
                                        .withAnnotations(singletonList(nullabilityAnnotation.withPrefix(Space.SINGLE_SPACE))));
                            } else {
                                m2 = m2.withReturnTypeExpression(new J.AnnotatedType(
                                        Tree.randomId(),
                                        Space.SINGLE_SPACE,
                                        Markers.EMPTY,
                                        singletonList(nullabilityAnnotation.withPrefix(Space.EMPTY)),
                                        m2.getReturnTypeExpression()
                                ));
                            }
                            m2 = autoFormat(m2, m2.getReturnTypeExpression(), ctx, getCursor().getParentOrThrow());
                            return m2.withPrefix(m2.getPrefix().withWhitespace(m2.getPrefix().getWhitespace().replace("\n\n\n", "\n\n")));
                        })
                        .orElse(m);
            }
        };
    ...
    private static boolean isNullabilityAnnotation(J.Annotation annotation) {
