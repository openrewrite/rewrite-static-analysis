...
public class NewStringBuilderBufferWithCharArgument extends Recipe {
    ...
    @Getter
    final String description = "Instantiating a `StringBuilder` or a `StringBuffer` with a " +
            "`Character` results in the `int` representation of the character " +
            "being used for the initial size. This is almost never the " +
            "developer's intent and silently produces a buffer with an arbitrary " +
            "capacity instead of the expected initial content.";
