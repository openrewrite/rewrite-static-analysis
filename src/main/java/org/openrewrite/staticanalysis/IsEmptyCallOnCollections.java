...
public class IsEmptyCallOnCollections extends Recipe {
    ...
    final String description = "Also check for _not_ `isEmpty()` when testing for not equal to zero size. " +
            "Using `isEmpty()` communicates intent more clearly than comparing `size()` to zero, and for some collection implementations `isEmpty()` can be more efficient since `size()` may require traversal.";
